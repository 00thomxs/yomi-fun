...
export function BadgesModal({ isOpen, onClose, userId }: BadgesModalProps) {
  ...
  const handleToggleEquip = async (userBadgeId: string) => {
    ...
    if (result.success) {
      ...
      toast.success(result.isEquipped ? 'Badge équipé !' : 'Badge déséquipé')
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col">
        ...
        {/* Category Tabs */}
        <div className="flex gap-1 p-3 border-b border-zinc-800 overflow-x-auto scrollbar-hide">
          {CATEGORIES.map(cat => (
            <button
              key={cat.key}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {ownedBadges.map(badge => {
                      ...
                      return (
                        <div
                          key={badge.id}
                          onClick={() => userBadge && handleToggleEquip(userBadge.id)}
                          className={cn(
