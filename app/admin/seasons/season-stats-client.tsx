...
import { Trophy, Target, Flame, TrendingUp, TrendingDown, Award, Users, Zap, BarChart3, Medal } from 'lucide-react'
...
export function SeasonStatsClient({ 
  ...
  const [activeTab, setActiveTab] = useState<'leaderboard' | 'stats' | 'events' | 'history'>('leaderboard')
  ...
  const stats = useMemo(() => {
    ...
    // Top 3 by Streak (approximated by consecutive wins ratio)
    const topStreak = leaderboard
      .filter(e => e.wins > 0)
  ...
  const positionChartData = useMemo(() => {
    ...
    // Group by date and get top 10 users
  ...
  const pnlDistribution = useMemo(() => {
    const ranges = [
      ...
      { name: '-10K à -1K', min: -10000, max: -1000 },
      { name: '-1K à 0', min: -1000, max: 0 },
      { name: '0 à 1K', min: 0, max: 1000 },
      { name: '1K à 10K', min: 1000, max: 10000 },
  ...
  const eventVolumeData = useMemo(() => {
    return events
      ...
      .slice(0, 5)
      .map(e => ({
        name: e.question.length > 30 ? e.question.substring(0, 30) + '...' : e.question,
        value: e.volume
      }))
  }, [events])
  ...
  const tabs = [
    ...
    { key: 'stats', label: 'Stats Joueurs', icon: Award },
    { key: 'events', label: 'Events', icon: Target },
    { key: 'history', label: 'Évolution', icon: TrendingUp },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'leaderboard' && (
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          <div className="px-4 py-3 bg-white/5 border-b border-border">
            <h3 className="font-bold text-sm">Classement Complet</h3>
          </div>
      ...
      {activeTab === 'events' && (
        <div className="space-y-6">
          {/* Volume by Event Chart */}
          {eventVolumeData.length > 0 && (
            <div className="p-4 rounded-xl bg-card border border-border">
              <h4 className="text-xs uppercase tracking-wider text-muted-foreground mb-4">Volume par Event (Top 5)</h4>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
          ...
          <div className="rounded-xl bg-card border border-border overflow-hidden">
            <div className="px-4 py-3 bg-white/5 border-b border-border">
              <h3 className="font-bold text-sm">Events de la Saison</h3>
            </div>
            <div className="divide-y divide-border">
              {events.map(event => (
                <div key={event.id} className="p-4 flex items-center justify-between hover:bg-white/5">
                  <div>
                    <p className="font-medium text-sm">{event.question}</p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(event.created_at).toLocaleDateString('fr-FR')}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-mono font-bold flex items-center gap-0.5 justify-end">
                      {event.volume.toLocaleString()}<CurrencySymbol className="w-3 h-3" />
                    </p>
                    <span className={`text-xs font-bold uppercase ${
                      event.status === 'open' ? 'text-emerald-400' :
                      event.status === 'closed' ? 'text-amber-500' : 'text-muted-foreground'
                    }`}>
                      {event.status === 'open' ? 'Live' : event.status === 'closed' ? 'Attente' : 'Terminé'}
                    </span>
                  </div>
                </div>
              ))}
          ...
          {positionChartData.length > 0 ? (
            <div className="p-4 rounded-xl bg-card border border-border">
              <h4 className="text-xs uppercase tracking-wider text-muted-foreground mb-4">
                Évolution des Positions (Top 10)
              </h4>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
              ...
              <p className="text-xs text-muted-foreground mt-2 text-center">
                Position 1 = Premier, Position 10 = Dixième
              </p>
            </div>
          ) : (
            <div className="p-8 rounded-xl bg-card border border-border text-center">
              <TrendingUp className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
              <h3 className="font-bold">Pas encore d'historique</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Les snapshots sont capturés après chaque résolution d'event de saison.
              </p>
            </div>
          )}
        </div>
